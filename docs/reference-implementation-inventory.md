...
This inventory separates real Mucha Science runtime integrations from gaps that
...
- `clean-room implementation`: local Mucha Science code implements the behavior
...
what Mucha Science can honestly claim today. Stage claims must use these labels, not
...
- `MiroFish` is documented as AGPL-3.0. Current behavior is a selected local
  ...
  because Mucha Science emits a MiroFish-style world graph, agent environment,
...
- `GBrain` was rechecked against the upstream repository during the 2026-05-02
  ...
  `license/compliance blocked` status is no longer used. Mucha Science now treats
...
- `HACHIMI` was rechecked against `ZeroLoss-Lab/HACHIMI` during the 2026-05-03
  audit and its upstream `LICENSE` is MIT. Mucha Science treats it as a local
...
three runtime phases Mucha Science executes for each council round: private
...
before final persona admission. The implementation remains a Mucha Science-specific
