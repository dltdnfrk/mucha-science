...
pub(super) fn validate_envelope(object: &Map<String, Value>) -> Result<(), String> {
    ...
    if object.get("protocol_version").and_then(Value::as_str) != Some(SCIENTIFIC_PROTOCOL_VERSION) {
        return Err(format!(
...
fn validate_welcome(object: &Map<String, Value>) -> Result<(), String> {
    ...
    if payload.get("protocol_version").and_then(Value::as_str) != Some(SCIENTIFIC_PROTOCOL_VERSION)
