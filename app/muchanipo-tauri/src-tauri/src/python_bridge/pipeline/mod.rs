...
use super::{
    ...
    state::{
        ...
        owner_boot_id, signal_verified_process_group, write_cancel_token, CancellationDecision,
        ExecutionOwner, LaunchDecision, LaunchRequest, PythonBridge,
    },
...
pub async fn start_pipeline(
    ...
    let decision = execution
        .owner
        .as_mut()
        .ok_or_else(|| "execution owner was not initialized".to_string())?
    ...
    let child_handle = execution
        .runtime
        .as_ref()
        .ok_or_else(|| "pipeline runtime was not installed".to_string())?
        .child
        .clone();
    drop(execution);
