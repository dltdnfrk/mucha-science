...
impl ExecutionOwner {
    ...
    pub(crate) fn recover(
        ...
        match (handshake, observed) {
            ...
            (None, None)
                if now_unix_ms.saturating_sub(receipt.reserved_at_unix_ms) < RESERVED_GRACE_MS =>
            {
    ...
    pub(crate) fn request_cancel(
        ...
        if current.phase == ReceiptPhase::CancelRequested {
            return Ok(CancellationDecision::AwaitTermination);
        }
    ...
    pub(crate) fn observe_termination(
        ...
        self.terminalize(app_run_id, generation, TerminalKind::Canceled, true, reaped)
    }
