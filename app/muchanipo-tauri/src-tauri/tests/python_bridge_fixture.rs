...
fn temporary_directory(name: &str) -> PathBuf {
    ...
    let directory = std::env::temp_dir().join(format!(
        "muchanipo-python-bridge-{name}-{}-{nonce}",
        std::process::id()
    ));
...
fn packaged_resolver_selects_the_generic_external_bin_in_the_packaged_layout() {
    ...
    fs::write(resource_dir.join("caller-selected.pyz"), b"untrusted")
        .expect("arbitrary fixture should be written");
    ...
    assert_eq!(
        ...
        bundled_sidecar.canonicalize().unwrap()
    );
...
fn packaged_resolver_fails_closed_when_the_bundle_is_missing() {
    ...
    assert!(
        resolve_packaged_sidecar_path(&resource_dir, &executable_dir)
            .expect_err("missing sidecar must not fall back")
            .contains("bundled scientific sidecar is missing")
    );
    fs::remove_dir_all(resource_dir).expect("temporary fixture directory should be removed");
    fs::remove_dir_all(executable_dir).expect("temporary fixture directory should be removed");
}
...
fn packaged_resolver_rejects_a_manifest_hash_mismatch() {
    ...
    let sidecar_name = scientific_sidecar_name();
    fs::write(executable_dir.join("muchanipo-service"), b"tampered")
...
fn packaged_resolver_rejects_a_symlink_even_when_it_points_inside_resources() {
    ...
    symlink(&target, executable_dir.join("muchanipo-service"))
...
fn start_reservation_rejects_duplicates_and_resets_for_a_new_generation() {
    ...
    let first = bridge
        ...
        .expect("first start reserves the bridge");
    assert_eq!(
        bridge.lifecycle_for_test().unwrap(),
        (BridgePhase::Starting, first)
    );
    assert!(bridge
        .reserve_start_for_test()
    ...
    bridge
        .reset_start_for_test(first)
        .expect("failed start resets its generation");
    let second = bridge
        .reserve_start_for_test()
        .expect("restart reserves a new generation");
    assert!(second > first);
    assert_eq!(
        bridge.lifecycle_for_test().unwrap(),
        (BridgePhase::Starting, second)
    );
}
#[test]
fn quarantined_generation_rejects_writes_and_new_starts_after_fatal_or_monitor_failure() {
    ...
    let generation = bridge
        .reserve_start_for_test()
        .expect("start should reserve a generation");
    ...
    assert_eq!(
        bridge.lifecycle_for_test().unwrap(),
        (BridgePhase::Quarantined, generation)
    );
    assert!(
        !bridge.accepts_writes_for_test().unwrap(),
        "quarantine must never authorize writes"
    );
    assert!(bridge
        .reserve_start_for_test()
...
fn quarantined_owned_process_is_retained_until_stop_retries_cleanup() {
    ...
    assert!(
        bridge.has_process_for_test().unwrap(),
        "quarantine retains cleanup ownership"
    );
    assert!(bridge
    ...
    assert_eq!(
        bridge.lifecycle_for_test().unwrap(),
        (BridgePhase::Stopped, generation)
    );
    assert!(
        !bridge.has_process_for_test().unwrap(),
        "successful retry releases ownership"
    );
}

#[test]
fn action_authorization_uses_only_current_negotiated_capabilities() {
    assert!(action_is_authorized_for_test(
        false,
        false,
        &[],
        "protocol.hello"
...
fn legacy_mode_accepts_legacy_actions_and_rejects_scientific_or_mixed_frames() {
    ...
    assert!(
        parse_backend_line_for_test(false, r#"{"event":"progress","protocol":"muchanipo"}"#)
...
fn scientific_mode_rejects_legacy_frames() {
    ...
    assert!(
        parse_backend_line_for_test(true, r#"{"event":"progress","protocol":"muchanipo"}"#)
...
fn oversized_jsonl_frames_are_rejected_before_unbounded_allocation() {
    ...
    assert_eq!(
        bounded_line_for_test(b"{\"event\":\"ok\"}\n", 64).unwrap(),
        Some("{\"event\":\"ok\"}".to_string())
    );
}
