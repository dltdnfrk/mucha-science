...
from .web.protocol_handler import ProtocolHandler
from .web.scientific_config import (
    ScientificConfigError,
    _advertised_capabilities,
    _approved_import_roots,
    _load_scientific_config,
    _scientific_config,
)
...
def scientific_serve(
    *,
    ...
    handler = ProtocolHandler(repository=repository, scientific_config=scientific_config)
    while line := stdin.readline():
        for out in handler.handle_line(line):
            stdout.write(out + "\n")
        # Pre-refactor emit helpers flushed every envelope; interactive stdio
        # consumers (Tauri sidecar) stream responses, so flush per input line.
        stdout.flush()
    return 0
